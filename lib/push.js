...
Push.prototype.sendMessages = function(identifier, json, cb) {
	...
	this.db.getTokens(identifier, json.tags, (err, tokens) => {
		...
		notificationService.send(notification, tokens).then( (response) => {
			...
			for (let failed of response.failed) {
				let token = failed["device"]
				let statusCode = failed["status"]
				if (statusCode === "400" && failed["response"]["reason"] === "BadDeviceToken") {
			...
			if (tokensToDelete.length > 0) {
				console.log("need to delete:", tokensToDelete);
				
				let deleteTasks = [];
				
				for (let token of tokensToDelete) {
					deleteTasks.push((cb) => {
						this.db.deleteToken(identifier, token, (token) => {
							cb(null);
						});
					});	
				}
